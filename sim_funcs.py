...
def conv_lunar_to_ecliptic(x,y,z,vx,vy,vz):
    x_rot = x
    y_rot = y * np.cos(MOON_OBLIQ) - z * np.sin(MOON_OBLIQ)
    z_rot = y * np.sin(MOON_OBLIQ) + z * np.cos(MOON_OBLIQ)
    vx_rot = vx
    vy_rot = vy * np.cos(MOON_OBLIQ)
    vz_rot = vy * np.sin(MOON_OBLIQ)
